# LLM Conversational Attractors

When two LLMs converse freely, they converge to "attractors": characteristic topics or behavioral patterns. This experiment generates LLM-to-LLM conversations and analyzes such attractors.

## Pipeline

**1. Generate transcripts** — each model talks to itself for 30 turns, starting with "Hello." and no system prompt. We generate 5 conversations per model and save them. Transcripts include `<think>` blocks (chain-of-thought reasoning) for models that produce them. We test 16 different LLMs.

```bash
python llm_dialogue.py --generate

# or run a single conversation with streaming output
python llm_dialogue.py -m google/gemini-3-flash-preview --turns 30
```

**2. Analyze** — an LLM judge (gemini-3-flash-preview) reads each transcript and summarizes the attractor in 3 sentences. Then, for each model, the 5 summaries are distilled by a second LLM judge (claude-opus-4.6) into a single-sentence characterization of the attractor. `<think>` blocks are stripped before analysis, use `--keep-thinking` to include them.

```bash
python attractor_analysis.py            # both stages
python attractor_analysis.py --stage1-only  # per-transcript summaries → analysis.txt
python attractor_analysis.py --stage2-only  # per-model synthesis → report.txt
```

## Results

> 80 transcripts across 16 models (5 conversations per model, 30 turns each).
>
> **Stage 1**: Per-transcript summaries by gemini-3-flash-preview → [`analysis.txt`](analysis.txt)
>
> **Stage 2**: Per-model synthesis by claude-opus-4.6 → [`report.txt`](report.txt)
